	...
	while (std::getline(in_file, line))
	{
		...
		else if (prefix == "f")
		{
			...
			while (ss >> temp_glinit)
			{
				...
				if (counter == 0) vertex_position_indicies.push_back(temp_glinit);
				else if (counter == 1) vertex_textcoord_indicies.push_back(temp_glinit);
				else if (counter == 2) vertex_normal_indicies.push_back(temp_glinit);
